...
public class VanillaClient {
    public static void main(String[] args) throws IOException, InvalidArgumentException, TransactionException, NetworkConfigurationException, InterruptedException {
        ...
        Gateway gateway = builder.identity(wallet, userName)
                .networkConfig(connectionProfile)
                .discovery(false)
                .connect();


        HFClient client = createClient(gateway.getIdentity());
        NetworkConfig networkConfig = NetworkConfig.fromYamlFile(Paths.get("gateway", "networkConnection.yaml").toFile());
        ...
        Log log = LogFactory.getLog(VanillaClient.class);
